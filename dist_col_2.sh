...
for filein in $(find -name totg0.dat); do 
...
count=1
for ion in $(grep zion ./q1_at1/o*.out | awk '{print $3}'); do
    zion_type[$count]=$ion
    let count=count+1
done
count=1
for ion in $(grep " typat" ./q1_at1/o*.out | tail -n1); do
    if (( $count>1 )); then
        ct=$((count-1))
...
natom=$(grep -w natom q1_at1/o*.out | tail -1 | awk '{print $2}')
