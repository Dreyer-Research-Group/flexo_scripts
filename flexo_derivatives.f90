program flexo_derivatives
  implicit none
  integer :: nlines,ilines,iq,jq,ialpha,iat,idir,z_ct,ii,natom
        ...
        do iq=1,3 
           ...
           do jq=1,3
              ...
              if (abs(qvec(jq,ilines+1)) > 1.0d-6 .and. abs(qvec(iq,ilines+1)) > 1.0d-6) then
                 ...
                 ! TEST
                 !write(*,*) at(ilines),ialpha,dir(ilines),iq,jq
                 !write(*,*) pind(2*ialpha,ilines),-2.pind(2*ialpha,ilines+1),pind(2*ialpha,ilines+2)
                 !write(*,*) qvec(iq,ilines+1),qvec(jq,ilines+1)
                 !write(*,*)
...
!  stop
  ...
  ! Write output
  open(1,file="derivatives_piezo_2.dat")
  if (pz_convert==1) then
  ...
  ! Write output
  open(1,file="derivatives_flexo_2.dat")
